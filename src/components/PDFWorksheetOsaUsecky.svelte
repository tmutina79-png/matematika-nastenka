<script>
  ...
  const problems = [
    {
      id: 1,
      title: 'Střed úsečky a osa',
      text: 'Jsou dány body A a B, kde |AB| = 8 cm. Sestroj osu úsečky AB a vyznač její střed S. Ověř, že střed leží na ose.'
    },
    {
      id: 2,
      title: 'Bod na ose úsečky',
      text: 'Jsou dány body A a B, kde |AB| = 6 cm. Sestroj osu úsečky AB. Na ose vyznač bod X tak, aby |XA| = 5 cm. Ověř, že |XB| = 5 cm.'
    },
    {
      id: 3,
      title: 'Rovnoramenný trojúhelník',
      text: 'Je dána úsečka AB, kde |AB| = 7 cm. Najdi bod C tak, aby trojúhelník ABC byl rovnoramenný s |CA| = |CB| = 5 cm.'
    },
    {
      id: 4,
      title: 'Kružnice opsaná trojúhelníku',
      text: 'Je dán trojúhelník ABC s délkami stran a = 6 cm, b = 5 cm, c = 7 cm. Sestrojením os stran najdi střed kružnice opsané a kružnici narýsuj.'
    },
    {
      id: 5,
      title: 'Osa úsečky a přímka',
      text: 'Jsou dány body A, B (|AB| = 8 cm) a přímka p rovnoběžná s AB ve vzdálenosti 3 cm. Najdi na přímce p bod X, který je stejně vzdálený od A i B.'
    },
    {
      id: 6,
      title: 'Dvě osy úseček',
      text: 'Jsou dány tři body A, B, C neležící na jedné přímce (|AB| = 6 cm, |BC| = 5 cm, |AC| = 7 cm). Najdi bod X, který je stejně vzdálený od všech tří bodů. Jaká je vzdálenost |XA|?'
    }
  ];
  ...
  function generatePdf() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Osa úsečky – Příklady</title>
    <style>
    ...
    <h1>📐 Množina bodů – Osa úsečky – Pracovní list</h1>
